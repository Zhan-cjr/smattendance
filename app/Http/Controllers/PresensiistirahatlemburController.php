...
class PresensiistirahatlemburController extends Controller
{
    public function create()
    {
        ...
        return view('presensiistirahatlembur.create', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        $tanggal_selesai_istirahatlembur = $lintas_hari == 1 ? $tanggal_besok : $tanggal_sekarang;

        $in_out = $status == 1 ? "in" : "out";
        $image = $request->image;
        $folderPath = "public/uploads/istirahatlembur/";
        ...
        $jam_awal_istirahatlembur = $tanggal_presensi . " " . date('H:i', strtotime($jam_kerja->jam_awal_istirahatlembur));
        //Jam Mulai istirahatlembur 30 Menit Sebelum Jam istirahatlembur
        $jam_mulai_istirahatlembur = $tanggal_presensi . " " . date('H:i', strtotime('-' . $batas_jam_absen . ' minutes', strtotime($jam_awal_istirahatlembur)));

        //Jam Selesai istirahatlembur adalah 30 Menit Setelah Jam istirahatlembur
        $jam_mulai_akhir_istirahatlembur =  date('Y-m-d H:i', strtotime('+' . $batas_jam_absen . ' minutes', strtotime($jam_awal_istirahatlembur)));
        //return $jam_mulai_pulang;
        $jam_selesai_istirahatlembur = $tanggal_selesai_istirahatlembur . " " . $jam_kerja->jam_akhir_istirahatlembur;
        ...
        if ($status_lock_location == 1 && $radius > $cabang->radius_cabang) {
            ...
            if ($status == 1) {
                if ($presensi_hariini && $presensi_hariini->istirahatlembur_in != null) {
                    return response()->json(['status' => false, 'message' => 'Anda Sudah Absen Masuk Hari Ini', 'notifikasi' => 'notifikasi_sudahabsen'], 400);
                } else if ($jam_presensi < $jam_mulai_istirahatlembur) {
                    return response()->json(['status' => false, 'message' => 'Maaf Belum Waktunya Memulai istirahatlembur, Waktu istirahatlembur Dimulai Pukul ' . formatIndo3($jam_mulai_istirahatlembur), 'notifikasi' => 'notifikasi_mulaiabsen'], 400);
                } else {
                    try {
                        if ($presensi_hariini != null) {
                            Presensi::where('id', $presensi_hariini->id)->update([
                                'istirahatlembur_in' => $jam_presensi,
                                'lokasi_istirahatlembur_in' => $lokasi,
                                'foto_istirahatlembur_in' => $fileName
                            ]);
                        }
                        // Storage::put($file, $image_base64);

                        return response()->json(['status' => true, 'message' => 'Berhasil Memulai istirahatlembur', 'notifikasi' => ''], 200);
                    } catch (\Exception $e) {
                        return response()->json(['status' => false, 'message' => $e->getMessage()], 400);
                    }
                }
            } else {
                if ($presensi_hariini && $presensi_hariini->istirahatlembur_out != null) {
                    return response()->json(['status' => false, 'message' => 'Anda Sudah Mengakhiri istirahatlembur Hari Ini', 'notifikasi' => ''], 400);
                } else {
                    try {
                        if ($presensi_hariini != null) {
                            Presensi::where('id', $presensi_hariini->id)->update([
                                'istirahatlembur_out' => $jam_presensi,
                                'lokasi_istirahatlembur_out' => $lokasi,
                                'foto_istirahatlembur_out' => $fileName
                            ]);
                        }
                        // Storage::put($file, $image_base64);
                        return response()->json(['status' => true, 'message' => 'Berhasil Mengakhiri istirahatlembur', 'notifikasi' => ''], 200);
                    } catch (\Exception $e) {
                        return response()->json(['status' => false, 'message' => $e->getMessage()], 400);
                    }
                }
