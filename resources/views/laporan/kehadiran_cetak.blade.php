...
<head>
    ...
    <style>
        ...
        th { background: #024a75; color: white; font-weight: bold; }
...
<body>
    <h2 style="text-align:center;">Laporan Kehadiran Karyawan</h2>
    <div class="info">
        <strong>Periode:</strong> {{ \Carbon\Carbon::createFromFormat('Y-m-d', $tanggal_mulai)->timezone('Asia/Jakarta')->format('d/m/Y') }} s/d {{ \Carbon\Carbon::createFromFormat('Y-m-d', $tanggal_akhir)->timezone('Asia/Jakarta')->format('d/m/Y') }}<br>
        @if(request('kode_cabang'))
            <strong>Cabang:</strong> {{ request('kode_cabang') }}<br>
        @endif
        @if(request('nama_karyawan'))
            <strong>Nama Karyawan:</strong> {{ request('nama_karyawan') }}<br>
        @endif
        @if(request('kode_dept'))
            <strong>Departemen:</strong> {{ request('kode_dept') }}<br>
        @endif
        @if(($mode ?? 'detail') === 'rekap')
            <strong>Mode:</strong> Rekap
        @endif
    </div>
            ...
            <tbody>
                @forelse($rekapPresensis as $rekap)
                    <tr>
                        ...
                        <td class="center">{{ intval($rekap['jumlah_hari'] ?? 0) }}</td>
                        <td class="center">{{ intval($rekap['lembur'] ?? 0) }}</td>
                        <td class="number">{{ intval($rekap['kelebihan_menit'] ?? 0) }}</td>
                    </tr>
                @empty
                    <tr>
